...
room.subscribe('update',msg=>{
  ...
  if(p && p.alive){
    p.x=msg.data.x; p.y=msg.data.y; p.angle=msg.data.angle;
  }
});
...
setInterval(()=>{
  ...
  Object.values(bots).forEach(bot=>{
    ...
    if(Math.random()<0.01){
      const target=Object.values(players).filter(p=>p.alive && p.id!==bot.id)[Math.floor(Math.random()*Object.values(players).length)];
      if(target) bullets.push({x:bot.x,y:bot.y,vx:Math.cos(Math.atan2(target.y-bot.y,target.x-bot.x))*BULLET_SPEED*0.016,vy:Math.sin(Math.atan2(target.y-bot.y,target.x-bot.x))*BULLET_SPEED*0.016,owner:bot.id});
    }
  });
